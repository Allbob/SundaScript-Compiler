...
> **Konteks:** Dokumen ini menjelaskan rancangan, implementasi, dan validasi untuk **Fase 6: Code Optimization** ti kompilator SundaScript.
...
Modul pengoptimalan dijalankan melalui `src/optimizer.py` (`ASTOptimizer`) menggunakan mode *post-order traversal* untuk memodifikasi struktur pohon mulai ti lapisan terbawah (daun) ke lapisan tertinggi (akar).
...
- **Dead Code Elimination:** Mampu mendeteksi secara proaktif jalur kode yang tidak akan pernah tereksekusi secara teoritis (contoh: blok if dengan kondisi absolut `Salah`) dan menghapus rutenya secara langsung ti AST.
...
2. **Efisiensi:** Pohon AST yang sangat dalam berhasil direduksi secara drastis hingga **lebih ti 90%** ukuran aslinya dan menghasilkan keluaran instan dalam hitungan **milidetik**.
