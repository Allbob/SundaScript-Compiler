...
> **Konteks:** Dokumen ini menjelaskan rancangan, implementasi, dan validasi untuk **Fase 4: Abstract Syntax Tree** ti kompilator SundaScript.
...
- `src/ast_builder.py`: Mesin penelusur pohon yang secara cerdas mendeteksi *root node*, menelusuri ranting-rantingnya, mengeliminasi bagian token tak penting, lalu merakit ulang menjadi kelas node ti `ast_nodes.py`.
...
1. Pohon Mentah CST yang berasal ti Fase 3 harus tercetak secara eksplisit bersamaan dengan pohon murni AST ti Fase 4.
2. Hasil uji coba terbukti mampu mengkonversi blok kondisional `kok_lain` yang semula berisi **30+ token mentah** menjadi representasi kelas biner tunggal, mereduksi hingga lebih ti **60% noise tata bahasa**.
