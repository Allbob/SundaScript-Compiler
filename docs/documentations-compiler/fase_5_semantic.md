...
> **Konteks:** Dokumen ini menjelaskan rancangan, implementasi, dan validasi untuk **Fase 5: Semantic Analysis** ti kompilator SundaScript.

## Apa itu Fase 5 (Semantic Analysis)?
Fase 5 (Semantic Analysis) adalah sistem pemeriksa keabsahan dan kebenaran makna ti kode yang ada di dalam *Abstract Syntax Tree (AST)*, bertugas memastikan bahwa alur logika yang dirangkai masuk akal dan memenuhi hukum komputasi (seperti melarang penggunaan variabel yang belum dideklarasikan).
...
Sistem ini sangat mengandalkan komponen keamanan *Symbol Table* dalam melacak ruang lingkup (*Scope*). Setiap kali penelusur memasuki blok logika bersarang (seperti di dalam iterasi atau fungsi), operasi memori `push_scope` akan aktif, lalu ditutup dengan `pop_scope` begitu keluar ti kurung kurawal `{ }`.
