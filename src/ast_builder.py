...
class ASTBuilder:
    ...
    def build_function_def_stmt(self, node: ParseNode) -> FunctionDef:
        ...
        for param_token in params_node.children:
            if isinstance(param_token, Token) and (param_token.type.name == 'IDENTIFIER' or param_token.value == 'sorangan'):
    ...
    def build_try_stmt(self, node: ParseNode) -> TryStmt:
        ...
        while idx < len(node.children) and isinstance(node.children[idx], Token) and node.children[idx].value == 'iwal':
            idx += 1
            exc_type = None
            alias = None
            if isinstance(node.children[idx], Token) and node.children[idx].type.name == 'IDENTIFIER':
                ...
                idx += 1
                if isinstance(node.children[idx], Token) and node.children[idx].value == 'salaku':
        ...
        if idx < len(node.children) and isinstance(node.children[idx], Token) and node.children[idx].value == 'tungtungna':
            idx += 2 # skip tungtungna {
            finally_block = []
            while idx < len(node.children) and isinstance(node.children[idx], ParseNode):
    ...
    def build_match_stmt(self, node: ParseNode) -> MatchStmt:
        expression = self.build(node.children[1])
        cases = []
        idx = 3 # skip cocog expr {
        while idx < len(node.children) and isinstance(node.children[idx], Token) and node.children[idx].value == 'kaayaan':
            idx += 1
    ...
    def build_eureun_stmt(self, node: ParseNode) -> BreakStmt: return BreakStmt()
    def build_teras_stmt(self, node: ParseNode) -> ContinueStmt: return ContinueStmt()
    def build_liwatan_stmt(self, node: ParseNode) -> PassStmt: return PassStmt()
    ...
    build_pastikeun_stmt = _build_unary_ctrl
    build_hapus_stmt = _build_unary_ctrl
    build_sadayana_stmt = _build_unary_ctrl
    build_sanes_lokal_stmt = _build_unary_ctrl
    build_hasilkeun_stmt = _build_unary_ctrl
    ...
    def build_primary(self, node: ParseNode) -> ASTNode:
        ...
        if token.type.name == 'NUMBER':
            ...
            val = token.value
            if val == 'Leres': return Literal(True)
            elif val == 'Lepat': return Literal(False)
            elif val == 'Suwung': return Literal(None)
            elif val == 'sorangan': return Identifier('sorangan')
            elif val == 'indung': return Identifier('indung')
