...
class Parser:
    ...
    def parse_statement(self) -> ParseNode:
        ...
        if token.type == TokenType.IDENTIFIER:
            ...
            elif val == 'salami': return self.parse_while_stmt()
            elif val == 'pikeun': return self.parse_for_stmt()
            elif val == 'jieun': return self.parse_function_def_stmt()
            elif val == 'pulangkeun': return self.parse_return_stmt()
            elif val == 'kelas': return self.parse_class_def_stmt()
            elif val == 'coba': return self.parse_try_stmt()
            elif val == 'candak': return self.parse_import_stmt()
            elif val == 'ti': return self.parse_from_import_stmt()
            elif val == 'babarengan': return self.parse_async_stmt()
            elif val == 'cocog': return self.parse_match_stmt()
            elif val in ['eureun', 'teras', 'liwatan']: return self.parse_simple_ctrl_stmt(val)
            elif val in ['angkat', 'pastikeun', 'hapus', 'sadayana', 'sanes_lokal', 'hasilkeun']: return self.parse_unary_ctrl_stmt(val)
            elif val == 'sareng_ieu': return self.parse_with_stmt()
    ...
    def parse_if_stmt(self) -> ParseNode:
        ...
        while self.current_token().value == 'lamun_sanes':
            node.children.append(self.consume(TokenType.KEYWORD, "lamun_sanes"))
            node.children.append(self.consume(TokenType.LPAREN))
            node.children.append(self.parse_expression())
            node.children.append(self.consume(TokenType.RPAREN))
            self._parse_block(node)
    ...
    def parse_while_stmt(self) -> ParseNode:
        ...
        node.children.append(self.consume(TokenType.KEYWORD, "salami"))
        node.children.append(self.consume(TokenType.LPAREN))
        node.children.append(self.parse_expression())
        node.children.append(self.consume(TokenType.RPAREN))
        self._parse_block(node)
    ...
    def parse_for_stmt(self) -> ParseNode:
        ...
        node.children.append(self.consume(TokenType.KEYWORD, "pikeun"))
        node.children.append(self.consume(TokenType.LPAREN))
        node.children.append(self.consume(TokenType.IDENTIFIER))
    ...
    def parse_try_stmt(self) -> ParseNode:
        ...
        node.children.append(self.consume(TokenType.KEYWORD, "coba"))
        self._parse_block(node)
        while self.current_token().value == 'iwal':
            node.children.append(self.consume(TokenType.KEYWORD, "iwal"))
            if self.current_token().type == TokenType.IDENTIFIER:
                node.children.append(self.consume(TokenType.IDENTIFIER))
                if self.current_token().value == 'salaku':
                    node.children.append(self.consume(TokenType.KEYWORD, "salaku"))
                    node.children.append(self.consume(TokenType.IDENTIFIER))
            self._parse_block(node)
        if self.current_token().value == 'tungtungna':
            node.children.append(self.consume(TokenType.KEYWORD, "tungtungna"))
            self._parse_block(node)
        return node

    def parse_import_stmt(self) -> ParseNode:
        node = ParseNode("import_stmt")
        node.children.append(self.consume(TokenType.KEYWORD, "candak"))
        node.children.append(self.consume(TokenType.IDENTIFIER))
        if self.current_token().value == 'salaku':
            node.children.append(self.consume(TokenType.KEYWORD, "salaku"))
            node.children.append(self.consume(TokenType.IDENTIFIER))
    ...
    def parse_with_stmt(self) -> ParseNode:
        ...
        node.children.append(self.consume(TokenType.KEYWORD, "sareng_ieu"))
        node.children.append(self.parse_expression())
        if self.current_token().value == 'salaku':
            node.children.append(self.consume(TokenType.KEYWORD, "salaku"))
            node.children.append(self.consume(TokenType.IDENTIFIER))
    ...
    def parse_async_stmt(self) -> ParseNode:
        ...
        elif self.current_token().value == 'pikeun':
            node.children.append(self.parse_for_stmt())
        elif self.current_token().value == 'sareng_ieu':
            node.children.append(self.parse_with_stmt())
        else:
            raise ParserError("Expected 'jieun', 'pikeun', or 'sareng_ieu' after 'babarengan'", self.current_token())
        return node

    def parse_match_stmt(self) -> ParseNode:
        node = ParseNode("match_stmt")
        node.children.append(self.consume(TokenType.KEYWORD, "cocog"))
        node.children.append(self.parse_expression())
        node.children.append(self.consume(TokenType.LBRACE))
        while self.current_token().value == 'kaayaan':
            node.children.append(self.consume(TokenType.KEYWORD, "kaayaan"))
            node.children.append(self.parse_expression())
            self._parse_block(node)
    ...
    def _parse_params(self) -> ParseNode:
        ...
        if self.current_token().type != TokenType.RPAREN:
            ...
            if tok.type == TokenType.IDENTIFIER or (tok.type == TokenType.KEYWORD and tok.value == 'sorangan'):
    ...
    def parse_return_stmt(self) -> ParseNode:
        ...
        node.children.append(self.consume(TokenType.KEYWORD, "pulangkeun"))
    ...
    def parse_unary(self) -> ParseNode:
        if self.current_token().value == 'henteu':
            node = ParseNode("unary_expression")
            node.children.append(self.consume(TokenType.KEYWORD, 'henteu'))
        ...
        if self.current_token().type == TokenType.OPERATOR and self.current_token().value in ['-', '+']:
            ...
            node.children.append(self.parse_unary())
            return node
        if self.current_token().value == 'antosan':
            node = ParseNode("await_expression")
            node.children.append(self.consume(TokenType.KEYWORD, 'antosan'))
            node.children.append(self.parse_unary())
            return node
    ...
    def parse_primary(self) -> ParseNode:
        ...
        if token.type == TokenType.NUMBER:
            ...
            return node
        elif token.type == TokenType.KEYWORD and token.value in ['Leres', 'Lepat', 'Suwung', 'sorangan', 'indung']:
            node = ParseNode("primary")
