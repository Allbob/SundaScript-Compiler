...
class SymbolTable:
    def __init__(self):
        ...
        builtins = [
            'citak', 'panjang', 'himpunan', 'mutlak', 'Error', 
            'baca', 'buka', 'sleep', 'daptar', 'tipe', 'tanya',
            'angka', 'kecap', 'desimal', 'rentang'
        ]
        for b in builtins:
...
class SemanticAnalyzer:
    ...
    def visit_Identifier(self, node: Identifier):
        if node.name in ['sorangan', 'indung']:
    ...
    def visit_BreakStmt(self, node: BreakStmt):
        if self.loop_depth == 0:
            raise SemanticError("Sintaks 'eureun' (break) berada di luar perulangan.")

    def visit_ContinueStmt(self, node: ContinueStmt):
        if self.loop_depth == 0:
            raise SemanticError("Sintaks 'teras' (continue) berada di luar perulangan.")
